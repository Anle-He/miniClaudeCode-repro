...
Mini version: single template with a tool list and optional CLAUDE.md.
'''
...
def build_system_prompt(
    ...
    # The model learns its tools two ways: this human-readable list in the prompt (situational
    # awareness) AND the structured `tools=` schema (the actual call contract). Complementary.
    tool_list = '\n'.join(
        f'- **{t.name}**: {t.description}'
        for t in registry.all_tools()
    )

    # Optional project context: fold in CLAUDE.md if present, else leave the section empty.
    instructions = load_project_instructions(project_dir)
    project_section = ''
    if instructions:
        project_section = f'## Project Instructions (from CLAUDE.md)\n\n{instructions}'

    # The mode_description is a behavioural *hint* to the model; the hard block lives in
    # PermissionGate (stage 4). One advises, the other enforces.
