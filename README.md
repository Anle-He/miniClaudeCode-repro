...
从零复现 [miniClaudeCode](https://github.com/bcefghj/miniClaudeCode)（Claude Code 核心架构的蒸馏版）的学习项目。**当前进度：阶段二 · agent loop 主循环。**

> 目标是逐步复现一个具备 agent loop + 工具调用 + 权限 + 多轮上下文管理的 mini agent。本仓库按阶段提交，当前已含「地基 + 主循环」，后续阶段会陆续补齐。
...
| `miniclaudecode/tools/base.py` | **stub**：`ToolResult` + 假 `WeatherTool` + `ToolRegistry`（真实工具系统留待后续阶段） |
| `try_agent_loop.py` | 用假工具 `get_weather` 把循环骨架跑通的 runner |

> 标 **stub** 的模块是为了让主循环当前就能 import / 跑通的最小占位，后续阶段会替换成真实现。
